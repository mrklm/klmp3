...
# build-macos-high-sierra.sh — KLMP3 macOS High Sierra (Intel)
# Build: PyInstaller -> .app -> (optional) ZIP -> DMG -> sha256
#
# Usage:
#   ./build-macos-high-sierra.sh
#   ./build-macos-high-sierra.sh -v 2.4.1
#   ./build-macos-high-sierra.sh -v 2.4.1 --zip
#   ./build-macos-high-sierra.sh -v 2.4.1 --keep
#   ./build-macos-high-sierra.sh -v 2.4.1 --arch x86_64
#
# Assumptions:
# - Run at repo root (contains klmp3.py, assets/, tools/, .venv/)
# - assets/KLMP3.icns exists
# - tools/macos-<arch>/ffmpeg, ffprobe, qjs exist (executable)
...
for bin in ffmpeg ffprobe qjs; do
  [[ -f "${TOOLS_DIR}/${bin}" ]] || { echo "❌ Missing ${TOOLS_DIR}/${bin}"; exit 1; }
done

# Ensure the validated bundled tools are executable.
chmod +x "${TOOLS_DIR}/ffmpeg" "${TOOLS_DIR}/ffprobe" "${TOOLS_DIR}/qjs"

# A version check alone does not prove JavaScript can run on this OS.
"${TOOLS_DIR}/qjs" -e 'console.log("KLMP3 QuickJS:", 21 * 2)'

echo "== KLMP3 macOS High Sierra build =="
...
python -V
# High Sierra: conservation des outils Python du venv - python -m pip install -U pip wheel setuptools >/dev/null
...
pyinstaller \
  ...
  --add-binary "${TOOLS_DIR}/qjs:${TOOLS_DIR}" \
  --collect-all yt_dlp \
...
"${BUNDLE_TOOLS}/qjs" -e 'console.log("KLMP3 QuickJS:", 21 * 2)'
echo "✅ Tools embedded: ffmpeg, ffprobe, qjs"

# --- Release artifacts ---
mkdir -p releases

BASE_NAME="${APP_NAME}-${VERSION}-macOS-${ARCH}-high-sierra"
